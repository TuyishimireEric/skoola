...
import { getFilledStars } from "@/components/stars/StarRating";
...
const GameOverScreen = lazy(() => import("./RenderGameOverScreen"));
...
type QuestionType = keyof typeof QUESTION_TYPE_CONFIG;
...
export const RenderGameplay: React.FC<RenderGameplayProps> = memo(
  ({ questions, game, onBack, isCompleted, isLoading = false }) => {
    ...
    const handleGameCompletion = useCallback(() => {
      ...
      if (isAuthenticated && session?.user?.id) {
        const data: CourseSectionI = {
          ...
          Stars: getFilledStars(sessionData.score),
        };
        ...
        if (allMissedQuestions.length > 0) {
          submitMissedQuestions({
            studentId: session.user.id,
            questionIds: allMissedQuestions,
          });
        }
      } else {
...
RenderGameplay.displayName = "RenderGameplay";
